...
4. `scripts/compile.py` 调用本地 TeX Live（XeLaTeX）编译，生成 PDF。

本地需要：Python 3 和 TeX Live（含 xelatex、bibtex、Noto Serif CJK SC 字体）。不再依赖外部在线编译服务。
...
1. Clone 本仓库：
   ...
   ```bash
   git clone https://github.com/lazitech/arxiv-translator
   ```
...
| 依赖环境 | 各家工具形态不一 | 本地 XeLaTeX 编译，字体和宏包完全可控 |
...
│   ├── compile.py           # 使用本地 TeX Live (XeLaTeX) 编译，输出 PDF
│   └── cleanup.py           # 删除单篇论文的 .tmp_arxiv/<ID> 工作目录
└── references/
...
本 fork 在 [原版 arXiv-translator](https://github.com/Leey21/arxiv-translator) 基础上做了以下改动：

- **编译器**：LuaLaTeX → XeLaTeX，中文支持更成熟稳定
- **编译方式**：在线 API（latex.ytotech.com）→ 本地 TeX Live，不再依赖外部服务
- **依赖**：移除 `requests`（不再需要网络编译），需本地安装 TeX Live
- **预检**：新增 `\pdfoutput` 自动注释
